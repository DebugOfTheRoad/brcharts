...
seriesTypes.waterfall = extendClass(seriesTypes.column, {
	...
	translate: function () {
		...
		//处理分组显示
		maxHeight=0;
		isStreaming=series.chart.options.chart.subType=="streaming";
		if(isStreaming){
			groupKeys=series.chart.options.chart.groupKeys;
			maxHeightMap=new Map();
			for (var i = 0; i < groupKeys.length; i++) {
				maxHeightMap.put(groupKeys[i],points[i].shapeArgs.x);
			};
		}
		...
		for (i = 0, len = points.length; i < len; i++) {
			...
			if(isStreaming){
				shapeArgs.x=maxHeightMap.get(point.name);
			}
			...
			// up points
			previousY=point.start?point.start:0;
			...
			if (series.chart.inverted) {
				...
				if(isStreaming){
					point.tooltipPos[1]=series.xAxis.len - maxHeightMap.get(point.name);
				}
			} else {
	...
	processData: function (force) {
		...
		for (i = 0; i < dataLength; i++) {
			if(!window.memWaterMaxLength)
				start=this.options.data[i].start;
			else
				start=0;
			y = yData[i];
			if(start+y>max)
		...
		if(series.dataMin == undefined){
			if(!window.memWaterMaxLength){
				window.memWaterMaxLength=0;
				window.memWaterMinLength=0;
			}
			window.memWaterMaxLength+=max;
			window.memWaterMinLength=window.memWaterMinLength>min?min:window.memWaterMinLength;
		}else{
			window.memWaterMaxLength=0;
		}
		...
		// Record extremes
		series.dataMin=series.dataMin?series.dataMin:0;
		series.dataMax=series.dataMax?series.dataMax:0;
		series.dataMin =window.memWaterMinLength;
		series.dataMax += window.memWaterMaxLength;
	},
