...
SVGRenderer.prototype = {
	...
	getStyle: function (style) {
		return (this.style = extend({
			fontFamily: '"微软雅黑", Arial, Helvetica, sans-serif,"Lucida Grande", "Lucida Sans Unicode", Helvetica', // default font
			fontSize: '12px'
		}, style));
	},
		...
		each(axis.series, function (series) {
			...
			if (series.visible || !chart.options.chart.ignoreHiddenSeries) {
				...
				if (axis.isXAxis) {
					xData = series.xData;
					if (xData.length) {
						if(chart.options.chart.type=="waterfall"&&chart.options.chart.subType=="streaming"){
							axis.dataMin = 0;
							axis.dataMax = chart.options.chart.showLen-1;
						}else{
...
Legend.prototype = {
	...
	/**
	* Get the width of legend item
	*/

	getLegendItemWidth: function (item) {
		var legend = this,
			chart = legend.chart,
			options = legend.options,
			renderer = chart.renderer,
	...
	render: function () {
		var legend = this,
			...
			isAlignment=options.isAlignment||false,
			isFold=options.isFold||false;
		...
		if(isAlignment){
			...
			each(allItems, function (item) {
				maxItemWidth=mathMax(legend.getLegendItemWidth(item),maxItemWidth); 
			});
			options.itemWidth=maxItemWidth;
		}

		// render the items
		if(chart.options.chart.type=="waterfall"){
			for (var i = allItems.length-1; i >= 0; i--) {
				legend.renderItem(allItems[i]);
			};
		}else{
			each(allItems, function (item) {
		...
		if(isFold){
			legend.handleOverflow(legendHeight);
		}
...
/**
 * Set the default options for ring
 */
defaultPlotOptions.ring = merge(defaultSeriesOptions, {
	borderColor: '#FFFFFF',
	borderWidth: 1,
	center: [null, null],
	clip: false,
...
var RingPoint = extendClass(Point, {
	haloPath:function(){
		return null;
	}
});
 ...
 */
 var RingSeries = {
	...
	drawPoints: function () {
		...
		if(seriesOptions.or){
			bigR=seriesOptions.or;
		}else{
			bigR=((plotHeight>plotWidth?plotWidth:plotHeight)-20)/2;
		}
		if(seriesOptions.ir){
			smallR=seriesOptions.ir;
		}else{
			smallR=bigR/8
		}
		var rStep=( bigR-smallR )/len;
		var center=series.getCenter();
		var margin=seriesOptions.dataLabels.margin?seriesOptions.dataLabels.margin:10;

		for (var i = 0; i < points.length; i++) {
			points[i].outerR=bigR-rStep*i;
			points[i].innerR=bigR-rStep*(i+1);
		};
		...
		each(series.points, function (point) {
			...
			point.graphic = graphic = renderer.arc(series.center[0], series.center[1], point.outerR, point.innerR,(75-point.y)*Math.PI/50,Math.PI*3/2)
			.attr({
		        fill: point.color
		    })
		    .add(series.group);
	...
	drawDataLabels:function(){
		var series = this,
			...
			plotWidth= chart.plotWidth,
			renderer = chart.renderer,
			isCenter=seriesOptions.dataLabels.isCenter,
			points=series.points,
			len=points.length,
			bigR=0,
			smallR=0;
		if (seriesOptions.dataLabels.enabled || series._hasPointLabels) {
			...
			if(isCenter&&len==1){
				points[0].outerR=bigR;
				points[0].innerR=smallR;
				points[0].labelX=center[0];
				points[0].labelY=center[1];
			}else{
				for (var i = 0; i < len; i++) {
					points[i].outerR=bigR-rStep*i;
					points[i].innerR=bigR-rStep*(i+1);
					points[i].labelX=center[0]+Number(margin);
					points[i].labelY=center[1]-bigR+rStep*i+rStep/2;
				};
			}
			...
			each(series.points, function (point) {
				...
				var str=seriesOptions.dataLabels.formatter.call(point.getLabelConfig());
				...
				dataLabel.attr({
					x:isCenter?point.labelX-box.width/2:point.labelX,
					y:point.labelY-box.height/2
				});
			});
		}
	}
